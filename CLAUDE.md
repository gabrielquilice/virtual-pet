...
A pixel-art desktop pet (dog, cat, maritaca, sea turtle, fish, guinea pig, penguin, snake, rabbit, cockatiel, fox, wolf or snail) written in Python 3.12+ with PySide6 (Qt 6), managed with uv. The stack is cross-platform, but the app is built and tested for Linux (KDE Plasma 6 on Wayland, through XWayland); a Windows build is made and tested under Wine, and the user saw it work on several Windows 11 PCs. The scope is deliberately small: the pet roams on its own, a click makes it sit (another click lets it roam), it can be dragged anywhere but never off the screen, and its right-click menu offers Sit/Walk (Sit/Fly for the maritaca and the cockatiel, Sit/Swim for the sea turtle and the fish, Coil up/Slither for the snake, Sit/Hop for the rabbit, Retreat into shell/Crawl for the snail), Turn around, Hide, Settings… (name, species and language) and Quit. Hide puts the pet away in the system tray, or, on desktops without one, until the app is opened again. The interface is in English or Brazilian Portuguese, in which the app is called Pet Virtual. It ships as an x86-64 Linux AppImage, built locally by `appimage/build.py`, and as a Windows zip, built on the same Linux machine by `windows/build.py` under Wine (there is no CI workflow). Run from source on Debian/Ubuntu, Qt's X11 backend needs `libxcb-cursor0`; the AppImage bundles it.
...
uv run windows/build.py      # dist/VirtualPet-<version>-windows-x64.zip under Wine, then a start of it
...
- **`behavior.py`** is the pet's brain, in plain Python with no Qt.
  ...
  - `turn_around()` is the menu's Turn around ("Virar" in Portuguese): it flips `Facing`. A walking pet takes the rest of its stroll the other way, mirrored so it keeps its slope, and stops at a wall that comes first (at once if it is against one: it only turns to face it). Standing, sitting or carried, it only faces the other way, and its next stroll still picks a direction at random.
...
- **Art pipeline: `sprites.py`, `species.py`, `pets/`, `icon.py`**
  ...
  - `Locomotion` says how the pet roams, which is what its WALKING animation shows. Its value is the menu text for roaming again ("Walk", "Fly", "Swim", "Slither", "Hop", "Crawl").
  - `sit_label` is the menu text for sitting down: "Sit", except the snake's "Coil up" and the snail's "Retreat into shell".
  ...
  - The rabbit is `rabbit` in code, settings and file name, and "Rabbit" in the UI: white, with pink inside its ears. It hops (`Locomotion.HOP`, "Hop", "Saltitar" in Portuguese): its WALKING animation is crouched, pushing off with its front paws up, then landing with its rump up, and every frame keeps a foot on the ground. Its SITTING animation is sitting up on its haunches, sniffing.
  - The cockatiel is `cockatiel` in code, settings and file name, and "Cockatiel" in the UI ("Calopsita" in Portuguese): the classic gray one, with a yellow face and crest and orange cheeks. It flies like the maritaca, and its SITTING animation is fluffed up on the ground with the crest down. Its "B" is the yellow of the face, not the gray of the body: the eye sits in the face, so blinking closes it into a line.
  - The fox is `fox` in code, settings and file name, and "Fox" in the UI ("Raposa" in Portuguese). It is a red fox: red-orange with a white bib, muzzle and tail tip, black stockings and black backs to its ears. It walks (trots) with its bushy tail raised, and its SITTING animation is sitting up with the tail curled around its front paws.
  - The wolf is `wolf` in code, settings and file name, and "Wolf" in the UI ("Lobo" in Portuguese). It is a gray wolf in a warm gray, with a darker saddle, a cream muzzle, throat and chest, and amber eyes: its E and H are both amber, so a blink still closes the eye into a dark line. It walks (trots), and its SITTING animation is sitting up like the dog, raising its snout to howl every 8.5 s (HOWL_A and HOWL_B are the sitting head turned up about 40 degrees, not stretched, the mouth two pixels open in HOWL_B).
  - The snail is `snail` in code, settings and file name, and "Snail" in the UI ("Caracol" in Portuguese): a garden snail, its slim grayish beige body coming out from under a caramel spiral shell that sits snug against it, its eyes at the tips of two long stalks (the far one darker). It crawls (`Locomotion.CRAWL`, "Crawl", "Rastejar" in Portuguese like the snake's "Slither") at 15 px/s, the slowest pet. Its WALKING animation keeps the shell and the tail still while the head stretches one pixel ahead and draws back: a shell moving back and forth in a window that glides along would jolt backwards.
  - Told to sit, the snail retreats into its shell ("Entrar na concha"): its SITTING animation is the shell resting on the ground, 3 rows lower than when it crawls, with two eye glints (E over H, in the dark N of the opening) peeking out, as every frame needs eye pixels to blink. When carried it hides in its shell the same way, its eyes darting back and forth (SIT_B, SIT_C).
...
- **`instance.py`**: one pet at a time. `acquire_lock()` is a `QLockFile` in the runtime folder, with `setStaleLockTime(0)`.
  ...
  - Windows' Python has no Unix sockets, so there (`WINDOWS`) the requests go through a named pipe, Qt's `QLocalServer`/`QLocalSocket` with `UserAccessOption`, named after the socket's path (`pipe_name()`). QtNetwork is imported only there: the AppImage leaves it out, as on Linux it links Kerberos and more.
  - On Windows Qt's runtime folder is the user's home, so the lock goes in the temp folder (`%LOCALAPPDATA%\Temp`). The settings land in `%LOCALAPPDATA%\virtual-pet\config.json` with no change.
...
- **`dialogs.py`**: `PetDialog` is the adoption dialog on first run. `SettingsDialog` is the same plus a Language field ("System default", then each language by its own name). `ask_for_changes()` takes and returns `Preferences(pet, language)`.
  - The pets are a grid of checkable buttons, `PETS_PER_ROW` (4) per row. Every button gets the size of the largest one: in a `QGridLayout`, fixed-width buttons of different widths squeeze their column to the narrowest and cut the longer names.
...
- **`windows/`** builds the Windows zip on the Linux machine, as PyInstaller doesn't cross-compile. The user has no Windows machine of their own: Wine is the build's only test. On 2026-09-26 the user ran the 0.4.0 zip on several colleagues' Windows 11 PCs, and everything worked: the pet stays on top, clicking it doesn't take the focus, and it hides in the tray and comes back. Windows 10 is untried.
  - `build.py` loads `appimage/build.py` as a module (`common`) for the version naming, the downloads and `full_build_licenses()`. It runs in the dev `.venv` (it needs Qt only to draw the icon).
  - Windows' Python is python-build-standalone's `install_only` build, pinned by `PYTHON_VERSION` and `PYTHON_RELEASE` and checked against the release's SHA256SUMS. `uv export` plus `uv pip install --target --python-platform x86_64-pc-windows-msvc` put the Windows wheels of `uv.lock` in `build/windows/site-packages`, which PyInstaller, run by `wine python.exe`, finds through `PYTHONPATH`.
  - Wine commands get the build's own prefix (`build/windows/wine`), no `DISPLAY` or `WAYLAND_DISPLAY`, and `WINEDLLOVERRIDES` turning off Mono, Gecko and `winemenubuilder.exe`. Without that last one, `wineboot` rewrote the user's Wine file associations and icons in `~/.local/share` (it happened once, and was put back).
  - Qt 6.11's Core imports 20 `ucnv_*` functions from Windows' own `icuuc.dll` (Windows 10 1903+), which Wine lacks. `stub_dll()` writes a DLL exporting them all as `xor eax, eax; ret` into the prefix's system32; the names come from Qt6Core.dll's imports, read with the Windows packages' pure-Python pefile. The stub never goes in the bundle.
  - `virtual-pet.spec` keeps the Windows, offscreen and style plugins, then only the DLLs linked from what is left, so OpenGL and most of the C++ runtime go. The exe is `VirtualPet.exe`, windowed, with the icon (`write_icon()`, PNGs in an .ico) and a version resource (`write_version_info()`).
  - Licenses: the bundle's `licenses/` gets the pet's GPL, Qt's LGPL, python-build-standalone's texts, PyInstaller's COPYING and `THIRD-PARTY-NOTICES.txt` (CRLF). The Visual C++ runtime DLLs have a notice without texts, as a GPL system library. A file from the Wine prefix (a Wine DLL) stops the build.
  - The smoke test starts the bundle offscreen under Wine with a Portuguese config in the prefix's profile, waits for the lock, starts it again (exit 0, "asked to show") and fails on "Could not load" or a traceback. Then `wineserver --kill`: no graceful quit is tested.
  - With `DISPLAY` set, the bundle also runs on the real desktop through Wine's X11 driver, which showed the pet drawn correctly with the Windows platform plugin.
  - Scaling that isn't whole (125%, 150%, common on Windows) draws uneven art pixels; it is known and documented, not fixed.
...
- `tests/test_species.py` enforces the art invariants for every species:
  ...
  - the lowest visible row is 25 for sitting, for standing unless the species swims, and for walking only if the species walks, slithers, crawls or hops;
...
- New or changed pet art, and the app icon, must be shown to the user as images first: enlarged poses, plus real size on dark and light backgrounds. Wait for approval before it touches the code. The gray tabby cat (redrawn with a round head in profile, dark eyes and a short muzzle, kicking when carried), the sea turtle (brown shell, green skin), the betta (blue body, red fins), the tricolor guinea pig (ginger with a white blaze and band, a black patch, big dark eyes), the gentoo penguin (black and white, an orange bill and feet, a white band over the eye, sitting back on its tail rather than squashed), the green snake (white marks on the back, a yellow belly, the head at the end of a diagonal neck rather than on top of a vertical one, the eye below a row of green and no mouth line), the white rabbit (pink inside the ears, a snow-white cottontail, hopping, and sitting up on its haunches rather than lying down), the gray cockatiel (a yellow face and a curved crest, orange cheeks, a white wing patch; chosen over a lutino, and without raising its crest at rest), the red fox (a white bib and tail tip, black stockings, the bushy tail raised; its front legs under its chest, and its stockings keep their width when it trots), the gray wolf (a warm gray coat with a darker saddle, a cream muzzle and chest, amber eyes; the howl is the sitting head turned up about 40 degrees rather than stretched, with the mouth clearly open, every 8.5 s) and the garden snail (a slim body under a snug caramel spiral shell, chosen over a fatter body and a shell apart from the head; eyes peeking out of the shell when it sits, and hiding in it when carried, rather than hanging out of it or drawing only its foot in) are the designs the user approved, and the smooth cream paw on the rounded orange tile is the icon the user chose (it replaced a 32×32 pixel-art paw, and pixel-art versions of the new paw were turned down).
- The AppImage is x86-64 Linux only. The Windows zip is built and tested under Wine, which can't show how the window behaves on Windows: after changes to the window, the tray or the build, it needs trying on a Windows PC again, which the user does (Windows 11 so far).
...
- `tests/test_instance.py` runs each test as on Linux (a Unix socket) and as on Windows (a named pipe, which works on Linux too) by setting `instance.WINDOWS`; the socket-only ones skip where Python has no Unix sockets. `tests/test_windows_build.py` checks the ICU stub's exports, the .ico, the version resource and the Wine paths without Wine.
- The whole suite, but the process tests, also runs under Wine on Windows' Python: with the Windows wheels in a `--target` folder (all groups) and `PYTHONPATH="<site-packages>;<src>"`, `wine python.exe -m pytest --no-cov -p no:cacheprovider -m "not process" --ignore=tests/test_i18n.py --ignore=tests/test_build.py`, in the build's prefix (it needs the ICU stub).
