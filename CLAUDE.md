...
A pixel-art desktop pet (dog, cat, maritaca, sea turtle, fish, guinea pig, penguin, snake, rabbit, cockatiel, fox or wolf) written in Python 3.12+ with PySide6 (Qt 6), managed with uv. The stack is cross-platform, but the app is built and tested for Linux (KDE Plasma 6 on Wayland, through XWayland); a Windows build exists but is experimental, made and tested only under Wine. The scope is deliberately small: the pet roams on its own, a click makes it sit (another click lets it roam), it can be dragged anywhere but never off the screen, and its right-click menu offers Sit/Walk (Sit/Fly for the maritaca and the cockatiel, Sit/Swim for the sea turtle and the fish, Coil up/Slither for the snake, Sit/Hop for the rabbit), Turn around, Hide, Settings… (name, species and language) and Quit. Hide puts the pet away in the system tray, or, on desktops without one, until the app is opened again. The interface is in English or Brazilian Portuguese, in which the app is called Pet Virtual. It ships as an x86-64 Linux AppImage, built locally by `appimage/build.py`, and as an experimental Windows zip, built on the same Linux machine by `windows/build.py` under Wine (there is no CI workflow). Run from source on Debian/Ubuntu, Qt's X11 backend needs `libxcb-cursor0`; the AppImage bundles it.
...
- `tests/test_species.py` enforces the art invariants for every species:
  ...
  - the lowest visible row is 25 for sitting, for standing unless the species swims, and for walking only if the species walks, slithers or hops;
...
- New or changed pet art, and the app icon, must be shown to the user as images first: enlarged poses, plus real size on dark and light backgrounds. Wait for approval before it touches the code. The gray tabby cat (redrawn with a round head in profile, dark eyes and a short muzzle, kicking when carried), the sea turtle (brown shell, green skin), the betta (blue body, red fins), the tricolor guinea pig (ginger with a white blaze and band, a black patch, big dark eyes), the gentoo penguin (black and white, an orange bill and feet, a white band over the eye, sitting back on its tail rather than squashed), the green snake (white marks on the back, a yellow belly, the head at the end of a diagonal neck rather than on top of a vertical one, the eye below a row of green and no mouth line), the white rabbit (pink inside the ears, a snow-white cottontail, hopping, and sitting up on its haunches rather than lying down), the gray cockatiel (a yellow face and a curved crest, orange cheeks, a white wing patch; chosen over a lutino, and without raising its crest at rest), the red fox (a white bib and tail tip, black stockings, the bushy tail raised; its front legs under its chest, and its stockings keep their width when it trots) and the gray wolf (a warm gray coat with a darker saddle, a cream muzzle and chest, amber eyes; the howl is the sitting head turned up about 40 degrees rather than stretched, with the mouth clearly open, every 8.5 s) are the designs the user approved, and the smooth cream paw on the rounded orange tile is the icon the user chose (it replaced a 32×32 pixel-art paw, and pixel-art versions of the new paw were turned down).
