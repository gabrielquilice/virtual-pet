...
A pixel-art desktop pet (dog, cat, maritaca, sea turtle, fish, guinea pig, penguin, snake or rabbit) written in Python 3.12+ with PySide6 (Qt 6), managed with uv. The stack is cross-platform, but the app is built and tested for Linux (KDE Plasma 6 on Wayland, through XWayland); a Windows build exists but is experimental, made and tested only under Wine. The scope is deliberately small: the pet roams on its own, a click makes it sit (another click lets it roam), it can be dragged anywhere but never off the screen, and its right-click menu offers Sit/Walk (Sit/Fly for the maritaca, Sit/Swim for the sea turtle and the fish, Coil up/Slither for the snake, Sit/Hop for the rabbit), Hide, Settings… (name, species and language) and Quit. Hide puts the pet away in the system tray, or, on desktops without one, until the app is opened again. The interface is in English or Brazilian Portuguese, in which the app is called Pet Virtual. It ships as an x86-64 Linux AppImage, built locally by `appimage/build.py`, and as an experimental Windows zip, built on the same Linux machine by `windows/build.py` under Wine (there is no CI workflow). Run from source on Debian/Ubuntu, Qt's X11 backend needs `libxcb-cursor0`; the AppImage bundles it.
