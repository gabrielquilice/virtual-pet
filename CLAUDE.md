...
- **`dialogs.py`**: `PetDialog` is the adoption dialog on first run. `SettingsDialog` is the same plus a Language field ("System default", then each language by its own name). `ask_for_changes()` takes and returns `Preferences(pet, language)`.
  ...
  - `NoTrayNotice` is the non-modal message left by a pet that hides where there is no system tray: open the app again to bring it back. It's plain text, so a name like "<Rex>" isn't taken for markup.
...
- **`app.py`**
  ...
  - `PetController.hide_pet()` hides the window and shows the tray icon, or a `NoTrayNotice` where there is no tray; it asks at each hide, as a tray can come and go. `show_pet()`, from the tray or a later start, undoes both. Hiding isn't saved: each start shows the pet. The controller takes `tray_available`, so tests choose whether there is a tray.
